...
Run the deterministic, read-only upgrade-readiness audit for the live Pangolin stack on `vm890` and write a report under `docs/reports/`.
...
The target host, stack path, canonical URL, helper path, and current runtime state were re-verified by the planner on `2026-06-17`, and the upstream release sources used by this workflow are official documentation and official Git tags.
...
4. `README.md`
5. `specs/pangolin-upgrade-readiness-audit-v1.vm890.conf`
...
- The live stack currently runs `fosrl/pangolin:1.19.2`, `fosrl/gerbil:1.4.2`, `traefik:v3.6.16`, and Badger plugin `v1.4.1`
...
- the report states current runtime, current-line latest, and overall latest
- the report includes a read-only proposed version path and breaking-change/gotcha notes
- the report is written under `docs/reports/`
...
- the official upstream tag sources cannot be read
...
This workflow is for upgrade-readiness truth only. Do not extend it into upgrade execution.
