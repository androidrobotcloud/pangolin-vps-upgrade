...
expected_image="${GERBIL_IMAGE_REPO}:${EXPECTED_VERSION}"
compose_image="$(remote "cd '$STACK_PATH' && awk '/image:.*fosrl\\/gerbil:/ {print \$2; exit}' docker-compose.yml")"
runtime_image="$(remote "docker inspect gerbil --format '{{.Config.Image}}'")"
...
printf '[pass] Gerbil compose and runtime image both match %s\n' "$expected_image"
