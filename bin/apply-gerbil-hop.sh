...
current_compose_image="$(remote "cd '$STACK_PATH' && awk '/image:.*fosrl\\/gerbil:/ {print \$2; exit}' docker-compose.yml")"
current_runtime_image="$(remote "docker inspect gerbil --format '{{.Config.Image}}'")"
target_image="${GERBIL_IMAGE_REPO}:${TARGET_VERSION}"
...
def parse(v: str):
    return tuple(int(p) for p in v.lstrip("v").split("-", 1)[0].split(".")[:3])
...
if [[ "$current_runtime_image" == "$target_image" ]]; then
  echo "[pass] Gerbil already at $target_image"
  exit 0
fi
...
remote "cd '$STACK_PATH' && docker compose pull gerbil"
remote "cd '$STACK_PATH' && docker compose up -d"

printf '[pass] Applied Gerbil hop to %s\n' "$TARGET_VERSION"
